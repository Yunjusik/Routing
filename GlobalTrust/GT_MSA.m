...
alpha = 0.9;                    % probability that malicious node drops a packet
        ...
        for m = 1:(2*ttime);
            ...
            if k ~= l && dist <= 250
                for n = 1:50
                    a = rand(2,1); 
                    % honesty node
                    if node(l,2) == 0 && a(1) <= forwarding
                        if a(2) < 0.95
                            p_events(k,l) = p_events(k,l) + 1;
                        else
                            n_events(k,l) = n_events(k,l) + 1;
                        end
                    elseif node(l,2) == 0 && a(1) > forwarding
                       if a(2) < 0.95
                           n_events(k,l) = n_events(k,l) + 1;
                       else
                           p_events(k,l) = p_events(k,l) + 1;
