...
z=50; %z는 랜덤성을 최소화하기 위한 iteration 숫자
...
for mp = 0.3 : 0.05 : 0.65
    
for i = 1: z 
    i
    [SR,GR,d] = MGT_BLA(noOfNodes-1,node_data, mp);
    
        for n = 60 : 60 : Time %라운드 반복
            
            for s = 1:sink-1 % 소스 1부터 92까지 변화를줌, Dest는 93
                n, s
                if (find(s == mali_node) >=1)
                else
                    [mal_ratio,S_ratio,mat] = Trust_routing22(SR,GR,noOfNodes,n,s,sink);
                    Count2 = Count2 +1;
