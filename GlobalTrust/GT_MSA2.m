function [FP_prob, FN_prob, TP_prob, d, RMSE] = GT_MSA2(alpha, mnode, Dist, decision)
...
while sum(node(:,2)) < mnode/2
    for k = 1:mnode
        half = rand(1,1);
        if node(mali_node(k),2) == 0
            if half > 0.5
                node(mali_node(k),2) = 1;
            end
        end
        if sum(node(:,2)) == mnode/2
            break;
        end
    end
end
...
% load gps data
for k = 1:N
    gps2 = fopen(sprintf('./KAIST/KAIST_30sec_0%02d.txt', k), 'r');
    temp = fscanf(gps2, '%g %g %g', [3 (2*ttime)]);
    ...
    for l = 1:N
        if isnan(BR_temp(l,k)) == 1
            ...
            if node(l,2) == 0
                ...
                BR_temp(l,k) = BR_temp(l,k) - (1 - alpha);
            end
        end
    end
end
