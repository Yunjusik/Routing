function [S_ratio] = Nround_routing_Show(noOfNodes,n,s,d)
...
global mali_node
 ...
 for i = 1:noOfNodes
     ...
     for j = 1:noOfNodes
           ...
           if distance <= R
               matrix(i, j) = distance;   %#ok<AGROW> % 이 매트릭스에 나중에 Cost를 삽입해야함, Cost는 SR(i,j)*GR(j)의 역수로 해야됨
               line([I_th_Xloc(i) I_th_Xloc(j)], [I_th_Yloc(i) I_th_Yloc(j)], 'LineStyle', ':');                         
           else
               matrix(i, j) = inf; %#ok<AGROW>
           end
      end
 end
